...
const folderDefinitions=[['brand','品牌与视觉设计'],['character','原创角色设计'],['film','人工智能影像'],['product','产品与交互设计'],['print','字体与平面设计'],['play','编程与互动作品'],['tarot','植物塔罗'],['about','关于我']];
const folderGuides={brand:'品牌识别、包装物料与空间应用。适合从视觉系统与应用延展了解作品。',character:'角色设定、表情动作与衍生应用。适合了解原创形象与风格系统。',film:'人工智能广告短片、叙事影像与制作过程。适合了解多媒体与动态叙事。',product:'智能产品、界面流程与交互原型。适合了解产品思考、信息组织与体验设计。',print:'字体实验、字形构建与印刷卡牌。适合了解版式、字形与平面表达。',play:'网页游戏、交互实验与可玩演示。适合了解把想法转化为实际体验的能力。'};
    ...
    <section class="profile-section" aria-labelledby="profile-projects">
     ...
     <div class="profile-project-grid">
      ${projects.map(item=>{const w=works[item.id];return `<article class="profile-project-card" id="profile-${item.id}">
       ...
       <div class="profile-card-links"><button class="profile-project-link" type="button" data-project="${item.id}">查看项目 ↗</button>${w.overviewEmbed?`<button class="profile-project-link" type="button" data-overview="${item.id}">${escape(w.overviewLabel||'观看项目简介 ↗')}</button>`:''}${w.interactiveEmbed?`<button class="profile-project-link" type="button" data-experience="${item.id}">${escape(w.interactiveLabel||'体验交互原型 ↗')}</button>`:''}</div>
      </article>`;}).join('')}
     </div>
...
function openExperienceModal(id,kind='interactive'){
 const w=works[id],overview=kind==='overview',embed=overview?w?.overviewEmbed:w?.interactiveEmbed;if(!embed)return;
 WorkShowcase.pause(projectDialog);
 const wide=overview?w.overviewWide:w.interactiveWide,src=withTheme(embed),plain=embed.split('?')[0],label=(overview?w.overviewTitle:w.interactiveTitle)||(wide?'互动展厅':'可交互手机原型');
 $('#project-type').textContent=(overview?w.overviewType:w.interactiveType)||'互动原型 / 可交互';
 $('#project-content').innerHTML=`<h2 id="project-title" class="sr-only">${escape(w.title)} · ${escape(label)}</h2><div class="demo-launch-bar"><span>${escape(label)}</span><a href="${escape(plain)}" target="_blank" rel="noopener">独立页面打开 ↗</a></div><iframe id="modal-game" class="world-game${wide?' world-exhibit':''}" src="${escape(src)}" title="${escape(label)}"></iframe>`;
 if(!projectDialog.open)projectDialog.showModal();syncModal();
}
