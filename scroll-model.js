...
function orbit(p,index,count){const travel=range(p,.15,.96);const angle=(index/count*360+170-travel*245)*Math.PI/180;const depth=(-Math.sin(angle)+1)/2;return{x:50+Math.cos(angle)*36,y:58+Math.sin(angle)*28,opacity:.72+depth*.28,angle};}
