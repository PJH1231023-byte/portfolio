...
(() => {
  ...
  const skies = [
    ...
    {id:'sunset',name:'地球日落',title:'地球日落 · 从太空望向家园',en:'ORBITAL SUNSET / EARTH',src:'images/sky/orbital-sunset-desktop.webp',srcset:'images/sky/orbital-sunset-mobile.webp 720w, images/sky/orbital-sunset-desktop.webp 1440w, images/sky/orbital-sunset-full.webp 1920w',full:'images/sky/orbital-sunset-full.webp',preview:'images/sky/orbital-sunset-preview.webp',fallback:'images/sky/orbital-sunset-fallback.jpg',position:'60% 50%',color:'#c38d73',note:'国际空间站拍摄的日落与云影，2022 年 11 月 13 日。',credit:'NASA; photograph by Koichi Wakata',url:'https://www.nasa.gov/image-article/cloud-shadows-stretch-across-earth-during-an-orbital-sunset/',alt:'蓝色大气沿地球的弧形地平线铺开，暖色夕照照亮云顶，云影延伸到远处。'}
  ];
  ...
  viewImage.addEventListener('error', () => {
    if (viewImage.dataset.fallbackUsed !== 'true' && viewImage.dataset.fallback) {
      viewImage.dataset.fallbackUsed = 'true';
      viewImage.src = viewImage.dataset.fallback;
    } else {
      viewImage.hidden = true;
      viewFeedback.textContent = '完整风景暂未载入，正在展示本地预览图。你仍可以返回浏览作品。';
    }
  });
  ...
  function setCredit(node, sky) {
    ...
    node.append('影像署名（背景已随网站保存）：',link);
  }
  function updateView(sky) {
    ...
    link.textContent = sky.credit + ' · 查看来源说明（外部网站）↗'; link.href = sky.url;
    viewImage.hidden = false;
    viewImage.dataset.fallback = sky.fallback;
    viewImage.dataset.fallbackUsed = 'false';
    dialog.querySelector('.sky-view-print').style.backgroundImage = 'url("' + sky.preview + '")';
    viewImage.src = sky.full; viewImage.alt = sky.alt;
  ...
  async function choose(sky, announce = true) {
    ...
    try {
      ...
      if (!entry) {
        ...
        image.fetchPriority = current ? 'auto' : 'high';
        image.sizes = '100vw';
        ...
        const ready = new Promise((resolve,reject) => {
          ...
          image.onerror = () => {
            if (!fallbackUsed) {
              ...
              image.src = sky.fallback;
              return;
            }
            image.remove(); images.delete(sky.id); reject(new Error('Local image unavailable'));
          };
        });
        entry = {image,ready}; images.set(sky.id,entry);
        stack.appendChild(image); image.srcset = sky.srcset; image.src = sky.src;
      }
      await entry.ready;
      if (token !== request) return;
      current = sky;
      background.style.backgroundImage = 'url("' + sky.preview + '")';
      ...
      if (token === request) {
        status.textContent = '完整风景暂未载入，作品仍可正常浏览。可以重试，或切换另一片风景。';
